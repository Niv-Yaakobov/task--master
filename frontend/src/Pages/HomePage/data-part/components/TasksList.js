...
const TaskList = ({ title, userId,allTasks,setAllTasks }) => {
    ...
    const { data, isPending, error } = useFetch(userId ? `https://task-master-backend-7g6m.onrender.com/${userId}/tasks` : null);
    ...
    const toggleTaskImportance = async (task) =>{
       ...
       await axios.patch(`https://task-master-backend-7g6m.onrender.com/${userId}/tasks/${task._id}`);
  ...
  const handleComplete = async (id) => {
    ...
    await axios.delete(`https://task-master-backend-7g6m.onrender.com/${userId}/tasks/${id}`);
  }
